...
| P2-10 | ✓ 2026-07-08 | Code quality | Native image reflection configuration | Plugin engines and script caches rely on reflection; add `reflect-config.json` and verify native build. Downgraded Groovy from `6.0.0-alpha-2` to `4.0.31` to match GraalVM's built-in substitution; removed `commons-compress` and migrated plugin ZIP read/write to `java.util.zip`; added `xstream` to satisfy Groovy AST parsing; added `META-INF/native-image/reflect-config.json` registering `PluginContext`, `Plugin`, and key script-engine classes; `mvn clean package -Pnative -DskipTests` now succeeds and the native executable starts. |
